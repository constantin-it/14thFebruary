...
pygame.display.set_caption("Love 'Em All: Тур примирения")
...
def show_menu(title, subtitle, btn_text):
    while True:
        ...
        btn_surf = font_main.render(btn_text, True, WHITE)
...
def main():
    ...
    while running:
        ...
        for h in hearts[:]:
            h.update()
            if james.rect.colliderect(h.rect) or dave.rect.colliderect(h.rect):
                score += 100
                thrash_value += 10
                hearts.remove(h)
                if thrash_value >= 100:
                    if os.path.exists(VICTORY_MUSIC):
                        try:
                            ...
                            pygame.mixer.music.play(-1, 34.0) # Начинаем с 34 сек
                        except:
                            pass
        ...
        pygame.draw.rect(screen, HEART_RED, (WIDTH//2 - 100, 20, thrash_value * 2, 20))
        thrash_lbl = font_ui.render("Трэш-метр", True, WHITE)
